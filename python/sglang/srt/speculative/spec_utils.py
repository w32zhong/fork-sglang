...
def select_top_k_tokens(
    ...
    if i == 0:
        ...
        scores = topk_logp.unsqueeze(-1) # shape: (b, topk, 1)
        tree_info = (
            topk_logp.unsqueeze(1),  # shape: (b, 1, topk), root is the single parent
            topk_index,  # shape: (b, topk)
            prebuilt_draft_index['init_parents']
        )
    else:
        ...
        expand_scores = scores + topk_logp # update path scores, shape: (b, topk, topk)
        ...
        if hidden_states.shape[0] > topk:
            # batched
            offset = prebuilt_draft_index['batch_select_offset']
