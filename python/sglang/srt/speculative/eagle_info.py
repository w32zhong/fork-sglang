...
class EagleDraftInput(SpecInput):
    ...
    prebuilt_draft_index: torch.Tensor = None
    ...
    def prepare_for_decode(
        self, logits_output: LogitsProcessorOutput, topk: int, bs: int
    ):
        ...
        # draft decode has its own inner loop and it is a hotspot that matters significantly for the overall
        # effectiveness of SD algorithm, we may pre-build draft indices here to minimize overall SD overheads.
        self.prebuilt_draft_index = dict(
            init_parents=torch.arange(-1, topk, dtype=torch.long, device="cuda").unsqueeze(0),
            batch_select_offset=torch.arange(0, bs * topk, step=topk, device="cuda").repeat_interleave(topk)
        )
