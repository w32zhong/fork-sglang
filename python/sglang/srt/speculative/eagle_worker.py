...
class EAGLEWorker(TpModelWorker):
    ...
    def draft_forward(self, forward_batch: ForwardBatch):
        ...
        assert isinstance(spec_info, EagleDraftInput)
        assert spec_info.prebuilt_draft_index is not None
        ...
        for i in range(self.speculative_num_steps):
            input_ids, hidden_states, scores, tree_info = select_top_k_tokens(
                i, topk_logp, topk_index, hidden_states, scores,
                self.topk, spec_info.prebuilt_draft_index
            )
    ...
    def forward_draft_extend(
        ...
        forward_batch.spec_info.prepare_for_decode(logits_output, self.topk, batch.batch_size())
